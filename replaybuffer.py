...
class ExperienceReplay:
  ...
  Arguments for instatiating the class:
    N (int): Maximum number of trajectories stored in memory list. Default value is 500
    batch_size (int): Number of trajectories to sample from memory list. Default value is 100
    DEVICE (str): Device to return batches of experiences for training
    """
  def __init__(self, N:int=5000, batch_size:int=128, DEVICE='cuda:0'):
    self.N = N
  ...
  def get_batch(self):
    ...
    state1_batch = torch.stack([x[0].squeeze(dim=0) for x in batch], dim=0).to(self.DEVICE)
    action_batch = torch.tensor([x[1] for x in batch]).long().to(self.DEVICE)
    reward_batch = torch.Tensor([x[2] for x in batch]).to(self.DEVICE)
    state2_batch = torch.stack([x[3].squeeze(dim=0) for x in batch], dim=0).to(self.DEVICE)
    terminated_batch = torch.Tensor([x[4] for x in batch]).to(self.DEVICE)
